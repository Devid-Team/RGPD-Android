...
class CGUActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (item in RGPD.shared.theme!!.pages) {
            if (item.pageName == RGPDPages.CGU.pageName) {
                theme = item
            }
        }
    ...
    fun onClickOnNextButton(view: View) {
        if (RGPD.shared.pagesShown.first().pageName === RGPDPages.CGU.pageName) {
            RGPD.shared.pagesAccepted.add(RGPDPages.CGU)
        ...
        if (RGPD.shared.pagesShown.count() == 0) {
            ...
            val intent = Intent(this@CGUActivity, RGPD.shared.pagesShown.first().activity)
    ...
    override fun onBackPressed() {
        super.onBackPressed()
        if (RGPD.shared.pagesAccepted.contains(RGPDPages.CGU)){
            RGPD.shared.pagesAccepted.removeAt(RGPD.shared.pagesAccepted.indexOf(RGPDPages.CGU))
            RGPD.shared.pagesShown.add(0, RGPDPages.CGU)
        }
    }
}