...
class CommActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (item in RGPD.shared.theme!!.pages) {
            if (item.pageName == RGPDPages.COMM.pageName) {
                theme = item
            }
        }
    ...
    private fun setColors() {
        ...
        if (theme.iconColor.size == 1) {
            ...
            icon.setImageDrawable(colorGenerator.setImageGradient(resources, R.mipmap.icon_commercial, theme.iconColor))
        }
    ...
    fun onClickOnNextButton(view: View) {
        if (RGPD.shared.pagesShown.first().pageName === RGPDPages.COMM.pageName) {
            RGPD.shared.pagesAccepted.add(RGPDPages.COMM)
        ...
        if (RGPD.shared.pagesShown.count() == 0) {
            ...
            val intent = Intent(this@CommActivity, RGPD.shared.pagesShown.first().activity)
    ...
    override fun onBackPressed() {
        super.onBackPressed()
        if (RGPD.shared.pagesAccepted.contains(RGPDPages.COMM)){
            RGPD.shared.pagesAccepted.removeAt(RGPD.shared.pagesAccepted.indexOf(RGPDPages.COMM))
            RGPD.shared.pagesShown.add(0, RGPDPages.COMM)
        }
    }
}