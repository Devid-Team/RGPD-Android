...
class PaymentInfoActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (item in RGPD.shared.theme!!.pages) {
            if (item.pageName == RGPDPages.PAYMENT_DATA.pageName) {
                theme = item
            }
        }
    ...
    private fun setColors() {
        ...
        if (theme.iconColor.size == 1) {
            ...
            icon.setImageDrawable(colorGenerator.setImageGradient(resources, R.mipmap.icon_payment, theme.iconColor))
        }
    ...
    fun onClickOnNextButton(view: View) {
        if (RGPD.shared.pagesShown.first().pageName === RGPDPages.PAYMENT_DATA.pageName) {
            RGPD.shared.pagesAccepted.add(RGPDPages.PAYMENT_DATA)
        ...
        if (RGPD.shared.pagesShown.count() == 0) {
            ...
            val intent = Intent(this@PaymentInfoActivity, RGPD.shared.pagesShown.first().activity)
    ...
    override fun onBackPressed() {
        super.onBackPressed()
        if (RGPD.shared.pagesAccepted.contains(RGPDPages.PAYMENT_DATA)){
            RGPD.shared.pagesAccepted.removeAt(RGPD.shared.pagesAccepted.indexOf(RGPDPages.PAYMENT_DATA))
            RGPD.shared.pagesShown.add(0, RGPDPages.PAYMENT_DATA)
        }
    }
}