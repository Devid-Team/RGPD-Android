...
class CGVActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (item in RGPD.shared.theme!!.pages) {
            if (item.pageName == RGPDPages.CGV.pageName) {
                theme = item
            }
        }
    ...
    fun onClickOnNextButton(view: View) {
        if (RGPD.shared.pagesShown.first().pageName === RGPDPages.CGV.pageName) {
            RGPD.shared.pagesAccepted.add(RGPDPages.CGV)
        ...
        if (RGPD.shared.pagesShown.count() == 0) {
            ...
            val intent = Intent(this@CGVActivity, RGPD.shared.pagesShown.first().activity)
    ...
    override fun onBackPressed() {
        super.onBackPressed()
        if (RGPD.shared.pagesAccepted.contains(RGPDPages.CGV)){
            RGPD.shared.pagesAccepted.removeAt(RGPD.shared.pagesAccepted.indexOf(RGPDPages.CGV))
            RGPD.shared.pagesShown.add(0, RGPDPages.CGV)
        }
    }
}